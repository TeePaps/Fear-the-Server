    ...
    public void connect(WifiP2pConfig config) {
        wifiManager.connect(channel, config, new WifiP2pManager.ActionListener() {
            ...
            public void onFailure(int reason) {
                Toast.makeText(MainActivity.this, "Connect failed. Search again.",
                        Toast.LENGTH_SHORT).show();
                disconnect();
            }
        });
...
//            PeerConnectFragment.newInstance(peerId);
    ...
    public void updateThisDevice(WifiP2pDevice device) {
        PrefsUtils.putString(this, PrefsUtils.KEY_MAC, device.deviceAddress);

        TextView view = (TextView) findViewById(R.id.tv_my_name);
        view.setText(device.deviceName);

        view = (TextView) findViewById(R.id.tv_my_mac);
